"""Remigrate legacy plate data using plate_type.
...
def _upsert_plate(conn, plate_columns: set[str], values: dict) -> int | None:
    ...
    conn.execute(
        sa.text(
            f"{prefix} INTO mkt_daily_plate ({', '.join(insert_columns)}) "
...
def _insert_plate_stock(conn, stock_columns: set[str], plate_id: int, stock: dict) -> None:
    ...
    insert_columns = [
        name for name in (relation_column, "stock_code", "stock_name", "change_pct", "last_price") if name in stock_columns
    ]
    prefix = _insert_ignore_prefix()
    conn.execute(
        sa.text(
            f"{prefix} INTO mkt_daily_plate_stock ({', '.join(insert_columns)}) "
            f"VALUES ({', '.join(f':{name}' for name in insert_columns)})"
        ),
        values,
    )
...
def _migrate_chance(conn, plate_columns: set[str], stock_columns: set[str]) -> None:
    ...
    for src in conn.execute(sa.text("SELECT * FROM mkt_daily_chance")).mappings().all():
        ...
        if plate_id and has_stock_table:
            for stock in conn.execute(sa.text("SELECT * FROM mkt_daily_chance_stock WHERE chance_id=:id"), {"id": src["id"]}).mappings().all():
...
def _migrate_tuyere(conn, plate_columns: set[str], stock_columns: set[str]) -> None:
    ...
    for src in conn.execute(sa.text("SELECT * FROM mkt_daily_tuyere")).mappings().all():
        ...
        plate_id = _upsert_plate(conn, plate_columns, values)
        if plate_id and has_stock_table:
            for stock in conn.execute(sa.text("SELECT * FROM mkt_daily_tuyere_stock WHERE tuyere_id=:id"), {"id": src["id"]}).mappings().all():
                _insert_plate_stock(conn, stock_columns, plate_id, stock)
...
def _migrate_limit_up(conn, plate_columns: set[str], stock_columns: set[str]) -> None:
    ...
    for src in conn.execute(sa.text("SELECT * FROM mkt_limit_up_plate")).mappings().all():
        values = {
            "trade_date": src["trade_date"],
            "plate_type": "limit_up",
            "platform": src.get("platform") or "cls",
